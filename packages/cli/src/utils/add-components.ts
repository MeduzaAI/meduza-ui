...
export async function addComponents(
  ...
  const spinner = silent
    ? null
    : logger.spin("Fetching components from registry...");
  ...
  const allDevDependencies = items.flatMap(
    (item) => item.devDependencies || [],
  );
  const allRegistryDependencies = items.flatMap(
    (item) => item.registryDependencies || [],
  );

  if (allDependencies.length > 0 || allDevDependencies.length > 0) {
    await updateDependencies(allDependencies, allDevDependencies, config, {
      silent,
    });
  }
  ...
  if (allRegistryDependencies.length > 0) {
    ...
    const dependenciesToInstall = await filterExistingDependencies(
    ...
    if (dependenciesToInstall.length > 0) {
      if (!silent) {
        logger.info(
          `Installing dependencies: ${dependenciesToInstall.join(", ")}`,
        );
      }
      // Don't overwrite existing dependency files to preserve user modifications
      await addComponents(dependenciesToInstall, config, {
        overwrite: false,
        silent: true,
        isInit,
      });
...
async function checkIfDependencyNeeded(
  ...
  const commonPaths = [
    ...
    path.resolve(
      ...
      `use${dependencyName.charAt(0).toUpperCase() + dependencyName.slice(1)}.ts`,
    ),
    path.resolve(
      config.resolvedPaths.composables,
      `use${dependencyName.charAt(0).toUpperCase() + dependencyName.slice(1)}.js`,
    ),
  ];
