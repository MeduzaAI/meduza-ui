...
async function promptForThemedColor(config: any): Promise<{ color: string; name: string }> {
    ...
    const colors = await getAvailableColors(registryUrl) as Array<{ name: string; label: string }>
    const themedColors = colors.filter((color: { name: string; label: string }) => color.name.includes('-')) // Themed colors have hyphens
...
async function listAvailableThemedColors(config: any) {
    ...
    const registryUrl = typeof registryConfig === "string" ? registryConfig : registryConfig?.url
    const colors = await getAvailableColors(registryUrl) as Array<{ name: string; label: string }>
    const themedColors = colors.filter((color: { name: string; label: string }) => color.name.includes('-')) // Filter for themed colors

    logger.break()
    logger.info(kleur.bold("Available themed colors:"))
    logger.break()

    themedColors.forEach((color: { name: string; label: string }, index: number) => {
