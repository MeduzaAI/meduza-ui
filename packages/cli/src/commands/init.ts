...
import { getAvailableColors, fetchColorData, createVariablesFile, createMixinsFile } from "@/registry/api"
import { type Config, type RawConfig, DEFAULT_REGISTRIES } from "@/registry/schema"
...
export async function runInit(options: z.infer<typeof initOptionsSchema>) {
    ...
    await injectColorVariables(config.baseColor || "slate", resolvedConfig, options)
    ...
    if (!options.silent) {
        ...
        logger.break()
        logger.info("2. The main.scss file has been automatically imported in your project")
    }
}
...
async function addMainScssImport(
    ...
    try {
        ...
        const relativePath = path.relative(path.dirname(mainFilePath), mainScssPath)
